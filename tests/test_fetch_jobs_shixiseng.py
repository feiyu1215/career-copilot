"""fetch_jobs_shixiseng.py 离线单测（纯 stdlib，无需真实网络 / bs4 / requests）。

覆盖：
- parse_jobs：字段提取（title/company/location/salary/url）+ URL 去重 + 退化链接扫描
- to_block：JOB_MATCHER_FORMAT v1 块格式
- 限流/风控检测 + 退避确定性
...
# 与 fetcher 的 `from job_common import` 对齐：先把 job_common 放进 sys.modules
...
spec = importlib.util.spec_from_file_location("fetch_jobs_shixiseng", SCRIPT)
fs = importlib.util.module_from_spec(spec)
sys.modules["fetch_jobs_shixiseng"] = fs
...
<div class="job-list-box">
  <div class="job-card">
    <a class="job-name" href="/intern/show/abc123">推荐算法实习生</a>
    <a class="company-name" href="/company/x">字节跳动</a>
    <span class="job-city">北京</span>
    <span class="job-money">200-300/天</span>
  </div>
  <div class="job-card">
    <a class="job-name" href="/intern/show/def456">后端开发实习生</a>
    <a class="company-name" href="/company/y">美团</a>
    <span class="job-city">上海</span>
  </div>
  <div class="job-card">
    <a class="job-name" href="/intern/show/abc123">重复条目</a>
  </div>
</div>
</body></html>
"""
...
def test_parse_jobs_extracts_fields_and_dedups():
    ...
    assert "https://www.shixiseng.com/intern/show/abc123" in urls
    assert "https://www.shixiseng.com/intern/show/def456" in urls
    assert urls.count("https://www.shixiseng.com/intern/show/abc123") == 1
    by = {j["url"]: j for j in jobs}
    a = by["https://www.shixiseng.com/intern/show/abc123"]
    assert a["title"] == "推荐算法实习生"
    assert a["company"] == "字节跳动"
    assert a["location"] == "北京"
    assert a["salary"] == "200-300/天"
    d = by["https://www.shixiseng.com/intern/show/def456"]
    assert d["title"] == "后端开发实习生" and d["company"] == "美团" and d["location"] == "上海"


def test_parse_jobs_fallback_link_scan():
    html = ('<html><body>'
            '<a href="/intern/show/zzz">产品实习生</a>'
            '<a href="/intern/show/zzz">产品实习生</a>'
            '<a href="/other/x">无关</a></body></html>')
    jobs = fs.parse_jobs(html)
    assert len(jobs) == 1
    assert jobs[0]["url"] == "https://www.shixiseng.com/intern/show/zzz"
    assert jobs[0]["title"] == "产品实习生"
